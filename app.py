...
# gcode = [i.strip() for i in open('this.gcode')]
...
# p = "32165458_70_2.png"
# val = p[p.index('_')+1:p.index('.')]
# w = p[p.index('_')+1:p.rindex('_')]
# h = p[p.rindex('_')+1:p.index('.')]
# print(w)
# print(h)
...
def PNG2Gcode(pngname, heightmm, widthmm, brallille):
    ...
    if brallille == True:
        ...
        p = numpy2stl(dots, "fun.stl", scale=0.05,
...
def STL2Gcode(p, filename):
    ...
    os.system(
        f"{pather}PrusaSlicer-version_2.4.2-armhf.AppImage -s {pather}{filename} -o {pather}this.stl")
